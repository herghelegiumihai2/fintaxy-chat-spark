...
const PricingSection = () => {
  ...
  const srlPlans: PricingPlan[] = [{
    ...
    title: "Personalizat",
    price: "",
    features: ["Daca nu te incadrezi in nici o categorie sau doresti o consultatie", "Dacă dorești să-ți aduci propriul contabil pe platforma Fintaxy"],
  ...
  const pfaPlans: PricingPlan[] = [{
    ...
    title: "Personalizat",
    price: "",
    features: ["Daca nu te incadrezi in nici o categorie sau doresti o consultatie"],
    cta: "Vreau să fiu sunat",
    formUrl: "https://airtable.com/appFj5aULmVgrYTpy/pagzTXzlTFmky6BKt/form"
  }];
  const infiintarePlans: PricingPlan[] = [{
    title: "SRL",
    price: "Gratuit",
    features: ["Înființare completă SRL", "Suport juridic", "Toate documentele necesare"],
    additionalInfo: ["Taxe Registrul Comerțului", "Înființare Gratuită Pentru toți antreprenorii care aleg și contabilitate"],
    cta: "Incepe Acum",
    highlighted: true,
    ...
    title: "PFA",
    price: "Gratuit",
    features: ["Înființare completă PFA", "Suport juridic", "Toate documentele necesare"],
    additionalInfo: ["Acte necesare:", "• Copie act de identitate", "• Copie a dovezii de spațiu + copii C.I. proprietari pentru sediul social", "• Documente care atestă calificarea în domeniul activității pe care urmează să o prestați"],
    cta: "Incepe Acum",
    formUrl: "https://airtable.com/appFj5aULmVgrYTpy/pag3oPTY2mZaISXFh/form"
  }];
  ...
  const scrollToContact = () => {
    ...
    if (isBusinessPage) {
      ...
      if (despreNoiSection) {
        despreNoiSection.scrollIntoView({
          behavior: 'smooth'
        });
        return;
      }
  ...
  const scrollCards = (direction: 'left' | 'right') => {
    if (cardsContainerRef.current) {
      ...
      const newScrollLeft = direction === 'left' ? cardsContainerRef.current.scrollLeft - scrollAmount : cardsContainerRef.current.scrollLeft + scrollAmount;
  ...
  return <section id="pricing" className="py-20 px-4 sm:px-6" ref={sectionRef}>
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="relative section-appear">
          ...
          <div className="hidden md:block">
            <button onClick={() => scrollCards('left')} className={cn("absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-full p-2 shadow-md transition-opacity", !canScrollLeft ? "opacity-0 pointer-events-none" : "opacity-100")} disabled={!canScrollLeft} aria-label="Scroll left">
              <ArrowLeft className="w-5 h-5 text-fintaxy-navy" />
            </button>
            
            <button onClick={() => scrollCards('right')} className={cn("absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-full p-2 shadow-md transition-opacity", !canScrollRight ? "opacity-0 pointer-events-none" : "opacity-100")} disabled={!canScrollRight} aria-label="Scroll right">
          ...
          <div className="flex justify-center">
            <div ref={cardsContainerRef} className="flex overflow-x-auto pb-6 scrollbar-hide snap-x snap-mandatory gap-4 md:gap-6 mx-auto justify-center" style={{
            scrollbarWidth: 'none',
            msOverflowStyle: 'none',
            maxWidth: '100%'
          }}>
              {getCurrentPlans().map((plan, index) => <div key={index} className="snap-center flex-shrink-0">
                  <Card className={cn("w-[280px] flex flex-col h-full min-h-[480px] transition-all duration-300", plan.highlighted ? 'border-2 border-fintaxy-blue ring-4 ring-blue-100 transform hover:-translate-y-1' : 'border border-gray-100 hover:border-fintaxy-blue hover:shadow-md')}>
                    <CardHeader className={cn("p-5", plan.highlighted ? 'bg-gradient-to-r from-fintaxy-blue to-blue-600 text-white' : 'border-b border-gray-100')}>
                      <h3 className="text-xl font-semibold mb-1">{plan.title}</h3>
                      {plan.subtitle && <p className={cn("text-sm mb-1", plan.highlighted ? 'text-blue-100' : 'text-fintaxy-muted')}>
                          {plan.subtitle}
                        </p>}
                      {plan.price && <div className="font-bold text-2xl">{plan.price}</div>}
                    </CardHeader>
                    <CardContent className="p-5 flex-1">
                      <ul className="space-y-3">
                        ...
                        {plan.additionalInfo && plan.additionalInfo.map((info, idx) => <li key={`info-${idx}`} className="text-fintaxy-muted text-xs mt-1.5">
                            {info}
                          </li>)}
                      </ul>
                    </CardContent>
                    <CardFooter className="px-5 pb-5 pt-0 mt-auto">
                      <Button size="sm" className={cn("w-full", plan.highlighted ? 'bg-gradient-to-r from-fintaxy-blue to-blue-600 hover:from-blue-600 hover:to-fintaxy-blue text-white' : 'border border-fintaxy-navy/20 bg-white text-fintaxy-navy hover:bg-fintaxy-light')} onClick={() => plan.cta === "Vreau să fiu sunat" ? scrollToContact() : window.open(plan.formUrl, '_blank')}>
                        {plan.cta}
                      </Button>
                    </CardFooter>
                  </Card>
